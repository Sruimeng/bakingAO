	...
	myRay* rays)
{
	...
	do
	{
		...
		u0 = rnd(seed);
		u1 = rnd(seed);
	} while (j < 5 && optix::dot(ray_dir, sample_face_norm) <= 0.1f);
...
// AO update kernel
__global__ void update_ao_kernel(int num_samples, HitInstancing* hit_data, float* ao_data)
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
...
__host__ void bake::update_ao_device(int num_samples, HitInstancing* hits, float* ao)
{
	int block_size = 512;
	int block_count = idiv_ceil(num_samples, block_size);
	update_ao_kernel << <block_count, block_size >> >(num_samples, hits, ao);
}
